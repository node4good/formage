...
module.exports = function (mongoose) {
    ...
    schema.statics.getByUsernamePassword = function (username, password, callback) {
        var self = this;
        self.findOne({'username': username}, function (err, admin_user) {
            if (err) throw err;
            if (!admin_user) return callback();
            if (!self.compareSync(password, admin_user.passwordHash)) return callback();
    ...
    return mongoose.model('_FormageUser_', schema);
};

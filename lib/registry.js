...
function ModelRegistry(adapter, models, options) {
    ...
    _.forEach(models, this._registerModel.bind(this));
}
...
ModelRegistry.prototype._registerModel = function (model, modelName) {
    ...
    process.nextTick(function () {
        filters.forEach(function (filter) {
            ...
            model.find(filter).limit(31).exec(function (err, results) {
                if (!results) return;
                results = _(results).pluck(filter).flatten().unique().valueOf();

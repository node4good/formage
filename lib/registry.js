...
ModelRegistry.prototype.getByUsernamePassword = function getByUsernamePassword(username, password) {
    if (username === this._superUser.username && UserForm.compareSync(password, this._superUser.passwordHash)) return Promise.fulfilled(this._superUser);
...
ModelRegistry.prototype.parseSession = function parseSession(sessionStore) {
    if (!sessionStore) return Promise.fulfilled();
    if (this._superUser.doubleHash === sessionStore) return Promise.fulfilled(this._superUser);

...
ModelRegistry.prototype.modelRegistrationInjection = function modelRegistrationInjection() {
    ...
    this.ref_fields = _(self.models).map(function (modelConfig) {
        ...
        modelConfig.fieldsHierarchy = AdminForm.buildFieldsHierarchy(self.adapter, modelConfig.model, LIST_EXCLUDED_FIELDS);
        modelConfig.allFields = AdminForm.getAllFields(modelConfig.fieldsHierarchy);
        return _.values(modelConfig.allFields);
    }).filter('ref').valueOf();
};
...
ModelRegistry.prototype.registerModel = function (model, modelName, adhoc_opts) {
    ...
    var modelConfig = this.models[modelName] = {
        ...
        populate: options.list_populate,
...
ModelRegistry.prototype.ensureUser = function ensureUser(username, password, callback) {
    this.adapter.UsersModel.ensureExists(username, password, function (err, adminUser) {
        if (err) throw err;

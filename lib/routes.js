...
module.exports = function (rootApp, registry) {
    var app = (rootApp._framework === 'opinion' ? createOpinionSubRouter : createExpressSubApp)(rootApp, registry);
...
function createExpressSubApp(rootApp, registry) {
    var hasRootExpress = rootApp.stack && _(rootApp.stack).pluck('handle').any({name: 'expressInit'});
    var express = hasRootExpress ? require.main.require('express') : require('express');
    var expressVer = hasRootExpress ? require.main.require('express/package.json') : require('express/package.json');
    var app = express();
...
function createOpinionSubRouter(rootApp, registry) {
    var common = require('opinion').common;
    var RouterCtor = Object.getPrototypeOf(rootApp.router).constructor;
    var router = new RouterCtor();
    var app = {middleware: [
        function* staticsMiddleware(next) {
            if (~this.originalUrl.indexOf(registry.root + '/vendor/ckeditor')) return yield common.statics(ckeditorPath)(next);
            if (~this.originalUrl.indexOf(registry.root)) return yield common.statics(path.join(__dirname, '..', 'assets'))(next);
        },
        router.middleware()
    ]};
    rootApp.mount(registry.root, app);
    return router;
}

...
function registerSockets(registry, cookieMiddleware) { // jshint ignore:line
    ...
    sio.set('authorization', function (request, callback) {
        ...
        var mockRes = {writeHead: _.noop};
        cookieMiddleware(mockReq, mockRes, _.noop);
        request.formageUser = mockReq.formageUser || (mockReq.session && mockReq.session.formageUser);
        var isAuth = !_.isEmpty(request.formageUser);
...
module.exports = function (rootApp, registry) {
    ...
    app.get('/fakeLogin', controllers.routes.fakeLogin);

...
module.exports = function (mongoose) {
    ...
    module.exports.getAdminUser = function (rawUser) {
        return rawUser ? module.exports.UsersModel.fromSessionStore(rawUser) : false;
    };
    ...
    module.exports.Users = models.FormageUser;
...
function queryDocuments(listFields, dbModel, filters, sorts, populates, start, count) {
...
function getFields(model, prefix) {
    var fields = paths_to_fields(model.schema.paths, model.schema.tree, prefix);
    fields = _.omit(fields, model.formageExclude);
    return fields;
}
...
function path_to_field(path, prefix, path_name, tree) {
    ...
    if (Array.isArray(fieldOptions.type)) {
        ...
        var generator = function (prefix) { return getFields({schema: schema, exclude: ['_id']}, prefix); };
        return new Fields.ArrayField(options, generator);
    }
    if (fieldOptions.ref) {
        if (fieldOptions.inline) {
            var refedModel = module.mongoose.model(fieldOptions.ref);
            var refFields = getFields(refedModel, prefix);
            return new Fields.InlineRefField(options, options.name + '.', refFields, refedModel);
        }
...
function getInnermostSchema(mongoose_field, tree) {
    ...
    if (inner_schema && (typeof(inner_schema) !== 'object' || inner_schema.type)) {
        ...
        _.assign(single_field, inner_schema, fieldOptions);

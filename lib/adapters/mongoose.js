...
module.exports = function (mongoose) {
    ...
    module.exports.getPaths = function (model) {
        var ret = _(model.schema.paths).map(function (obj, path) {
            ...
            options._path = path;
            options._typeName = options.type && options.type.name;
            return obj.options;
        });
        return ret;
    };
...
function queryDocuments(listFields, dbModel, filters,  sorts, populates, start, count, cb) {
    ...
    q.skip(start).limit(count).exec(function (err, docs) {
        if (err) throw err;
...
function mongoose_field_to_form_field(mongoose_field, name, tree) {
    ...
    var is_required = Boolean(fieldOptions.required);
    var def = fieldOptions['default'];
    var validators = [];
    var options = _.clone(fieldOptions);
    _.extend(options, {
        ...
        attrs: fieldOptions.attrs || {},
        widget: fieldOptions.widget
    });

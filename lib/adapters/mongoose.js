...
function queryDocuments(listFields, dbModel, filters, sorts, populates, start, count, cb) {
    ...
    q.skip(start).limit(count).select(listFields.join(' ')).exec().then(function (docs) {
        var filteredDocuments = docs.map(function (document) {
            var d = listFields.concat(['_id']).reduce(function (seed, listField) {
                seed[listField] = typeof(document[listField]) == 'function' ? document[listField]() : document[listField];
                return seed;
            }, {});
            return d;
        });
...
function getFields(model, exclude) {
    var fields = {};
    mongoose_fields_to_fieldsets(model.schema.paths, model.schema.tree, fields);
    fields = _.omit(fields, exclude);
    return fields;
}


function mongoose_fields_to_fieldsets(field_paths, field_tree, ref_fields) {
    Object.keys(field_paths).forEach(function mongoose_fields_to_fieldsets_inner(field_path) {
...
function mongoose_field_to_form_field(mongoose_field, db_path, label, tree) {
    ...
    if (Array.isArray(fieldOptions.type)) {
        ...
        var list_fields = {};
        mongoose_fields_to_fieldsets(schema.paths, schema.tree, list_fields);
        if (_.isEmpty(list_fields)) return null;
        return new Fields.ArrayField(options, list_fields);
    }
    if (fieldOptions.ref) {

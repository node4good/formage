...
module.exports = function (registry) {
    var parseQuery = function parseQuery(model_settings, query, search) {
    ...
    function buildPath(path) { return registry.root + path; }


    function readyForm(form, modelConfig, isNew, isDialog, callback) {
        form.pre_render(function onPreRender(err) {
            if (err) throw err;
            ...
            if (subCollections.length === 0) return callback(locals);
    ...
    function upsertDocumentHandler(req) {
        var modelName = req.params.modelName,
            docId = req.params.documentId,
            theDoc = req.params.theDoc,
            user = req.admin_user,
            data = _.extend({}, req.body, req.files);
        if (docId === 'new') docId = null;
        if (docId === 'single') docId = theDoc && theDoc.id;
        var returnPromise = new Promise();
        var modelConfig = registry.models[modelName];
        // We use a promise to be able to skip `find` in case don't have an ID
        var p = new Promise;
        if (docId) {
            modelConfig.model.findById(docId, p.resolve.bind(p));
        } else {
            process.nextTick(p.fulfill.bind(p));
        }
        p.then(function (document) {
            ...
            var form = new FormType(modelConfig.options, modelConfig.model, document, data);
            return form.save().then(
                function (document) {
                    document = modelConfig.options.post(document);
                    returnPromise.fulfill(document);
                },
                function (err) {
    ...
    var router = {
        json_routes: {
            ...
            upsertDocument: function upsertDocument(req, res) {
                upsertDocumentHandler(req).then(
                    function(document) {
                        res._debug_document = document;
                        res.json(200, document);
                    },
                    function (rejectedFrom) {
                        if (rejectedFrom) {
                            res._debug_form = rejectedFrom;
                            res.json(422, rejectedFrom.document);
                        }
                    }
                ).end();
            },
            ...
            actionDocuments: function actionDocuments(req, res) {
                var modelName = req.params.modelName,
                    ...
                    ids = req.body.ids;
                if (!req.admin_user.hasPermissions(modelName, 'update')) throw new Error('unauthorized');
                var action = _.find(registry.models[modelName].actions, {id: actionId});
                if (!action) throw new TypeError("Could not find action with id=" + actionId);
                return action.func(req.admin_user, ids, function (err, lines) {
        ...
        routes: {
            ...
            model: function model(req, res) {
                ...
                */
                ...
                var filters = parseQuery(modelConfig, query, search_value);
                ...
                return modelCounts(modelConfig, filters, function (err, total_count) {
                    ...
                    return listModelDocuments(name, start, count, filters, sort, function (documents) {
                        ...
                        var fieldLabel = function (fieldName) {
                            ...
                            var fieldConfig = paths.find({db_path: fieldName});
                        ...
                        var getTypeName = function getTypeName(fieldName) {
                            var fieldConfig = paths.find({db_path: fieldName});
                            return fieldConfig && fieldConfig.type.name;
                        };
            ...
            document: function document(req, res) {
                ...
                getDocument(function (err, document) {
                    ...
                    var form = new FormType(modelConfig.options, modelConfig.model, document, _.extend({}, req.body, req.files));
            ...
            documentPost: function documentPost(req, res) {
                ...
                upsertDocumentHandler(req).then(
                    ...
                    function (rejectedForm) {
                        readyForm(rejectedForm, rejectedForm.modelConfig, is_new, is_dialog, function (locals) {
                            res.status(422);
                            res.render("document.jade", locals)
                        });
                    }

...
module.exports = function (registry) {
    var parseQuery = function parseQuery(query, options, search) {
        ...
        var new_filters = _.reduce(
            ...
            function (seed, value, key) {
                ...
                var type = paths.find(key).type.constructor.name;
                switch (type) {
                    case 'String':
    ...
    function readyForm(form, modelConfig, isNew, isDialog, callback) {
        form.pre_render(function onRenderReady(err) {
            if (err) throw err;
    ...
    var router = {
        json_routes: {
            ...
            upsertDocument: function upsertDocument(req, res) {
                upsertDocumentHandler(req, function (rejectedFrom, modelConfig, document) {
                    if (rejectedFrom) return res.json(422, rejectedFrom.errors);
                    return res.json(200, {id: document.id, label: document.toString()});
                });
            },
        ...
        routes: {
            ...
            model: function model(req, res) {
                ...
                var filters = parseQuery(query, modelConfig, search_value);
                ...
                return modelCounts(modelConfig, filters, function (err, total_count) {
                    ...
                    return listModelDocuments(name, start, count, filters, sort, function (documents) {
                        ...
                        var getTypeName = function getTypeName(fieldName) {
                            var fieldConfig = paths.find({db_path: fieldName});
                            return fieldConfig && fieldConfig._typeName;
                        };
            ...
            document: function document(req, res) {
                ...
                getDocument(function (err, document) {
                    ...
                    if (is_new) {
                        var data = parseQuery(req.query, modelConfig);
                    }
            ...
            documentPost: function documentPost(req, res) {
                ...
                upsertDocumentHandler(req, function (rejectedForm, modelConfig) {
                    if (rejectedForm)

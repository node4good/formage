...
module.exports = function (registry) {
    ...
    function upsertDocument(req, modelName, docId, callback) {
        ...
        modelConfig.model.findById(docId, function (err, document) {
            ...
            var FormType = modelOptions.form;
            var form = new FormType(req, {instance: document, data: req.body}, model);
            form.init_fields();
            return form.is_valid(function (err, valid) {
                if (err) throw err;
                if (!valid) return callback(form, modelConfig);
                return form.save(function (err, document) {
                    if (err) throw err;
                    if (modelOptions.post) {
                        document = modelOptions.post(document);
                    }
                    return callback(null, modelConfig);
                });
            });
        });
    }
    ...
    var router = {
        json_routes: {
            ...
            upsertDocument: function (req, res) {
                upsertDocument(req, req.params.modelName, req.params.documentId, function (err, modelConfig) {
                    if (err) throw err;
                    res.json({id: doc.id, label:modelConfig.toString()});
                });
            },
        ...
        routes: {
            ...
            document: function (req, res) {
                ...
                id = (id === 'new') ? orig_id : id;
                var getDocument = (modelConfig.is_single) ? dbModel.findOne.bind(dbModel) : dbModel.findById.bind(dbModel, id);
                if (!id) getDocument = function (cb) {var d = new dbModel; process.nextTick(cb.bind(null, null, d));};

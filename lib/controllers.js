    ...
    var router = {
        ...
        authMiddleware: function (role) {
            return function (req, res, next) {
                var admin_user = formage.adapter.getAdminUser(req);
                if (!admin_user) {
...
var upsertDocument = function (req, res, admin, callback) {
    ...
    // Update
    if (doc_id) {
        admin.updateDocument(req, modelName, doc_id, req.body, function (err) {
            if (err) throw err;
            callback(modelConfig);
        });
    // Create
    } else {
        admin.createDocument(req, modelName, req.body, function (err) {
            if (err) throw err;
            callback(modelConfig);
        });
    }

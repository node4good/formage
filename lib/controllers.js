...
module.exports = function (admin) {
    function buildPath(path) { return admin.options.root + path; }

    function renderForm(res, form, modelConfig, allow_delete, clone, isDialog) {
    ...
    function listModelDocuments(modelName, start, count, filters, sort, callback) {
        var model_config = admin.models[modelName];
        ...
        return admin.adapter.queryDocuments(listFields, dbModel, filters, sorts, populates, start, count, callback);
    }
    ...
    var router = {
        ...
        routes: {
            ...
            model: function (req, res) {
                ...
                return admin.modelCounts(name, filters, function (err, total_count) {
                    ...
                    return listModelDocuments(name, start, count, filters, sort, function ( documents) {
                        ...
                        //noinspection JSUnresolvedVariable
                        var paths = admin.adapter.getPaths(modelConfig.model);
                        var fieldLabel = function (fieldName) {
                            if (!fieldName || !fieldName.length) return '';
                            var fieldConfig = _.find(paths, {_path: fieldName}) || {};
            ...
            document: function (req, res) {
                ...
                var dId = (id === 'new') ? orig_id : id;
                var getDocument = (modelConfig.is_single) ? dbModel.findOne.bind(dbModel) : dbModel.findById.bind(dbModel, dId);
            ...
            documentPost: function (req, res) {
                upsertDocument(req, res, admin, function (err, modelConfig) {
        ...
        authMiddleware: function (role) {
            return function (req, res, next) {
                var admin_user = admin.adapter.getAdminUser(req);
                if (!admin_user) {

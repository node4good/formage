...
function getDocument(modelConfig, id) {
    var Model = modelConfig.model,
        is_single = modelConfig.is_single,
        pops = modelConfig.populate;

    if (!is_single && (!id || id === 'new'))
        return Promise.fulfilled(new Model);

    var q = is_single ? Model.findOne() : Model.findById(id);
    if (!_.isEmpty(pops))
        q = q.populate(pops);
    return q.exec();
}
...
module.exports = function (registry) {
    ...
    function upsertDocument(req) {
        ...
        var p = getDocument(modelConfig, id).then(
    ...
    var router = {
            ...
            routes: {
                ...
                postLogin: function postLogin(req, res) {
                    registry.adapter.UsersModel.getByUsernamePassword(req.body.username, req.body.password, function (admin_user) {
                        ...
                        req.session._FormageUserID = admin_user.toSessionStore();
                ...
                logout: function logout(req, res) {
                    delete req.session._FormageUserID;
                    res.redirect('/');
                },
                ...
                document: function document(req, res) {
                    ...
                    getDocument(prelimConfig, id).then(
                        function (document) {
                            var docType = document && document._doc.__t;
                ...
                documentPost: function documentPost(req, res) {
                    ...
                    upsertDocument(req).then(
                        ...
                        function (err) {
                            ...
                            return err.form.pre_render(err.modelConfig, is_new, is_dialog).then(
                                function (locals) {
                                    res.status(422);
                                    res.render("document.jade", locals);
                                }
            ...
            authMiddleware: function authMiddleware(role) {
                return function (req, res, next) {
                    ...
                    if (!req.session) return next(new Error("Formage requires cookies to operate"));
                    registry.adapter.getAdminUser(req.session._FormageUserID).then(function (admin_user) {
                        if (!admin_user) {

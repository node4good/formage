    ...
    function upsertDocumentHandler(req, callback) {
        ...
        modelConfig.model.findById(docId, function (err, document) {
            ...
            return form.save(function (err, document) {
                if (err) {
                    callback(form, modelConfig, err);
                    return;
                }
                document = modelConfig.options.post(document);
                callback(null, modelConfig, document);
            });
        });
    }
    ...
    var router = {
        json_routes: {
            ...
            upsertDocument: function upsertDocument(req, res) {
                upsertDocumentHandler(req, function (rejectedFrom, modelConfig, document) {
                    if (rejectedFrom) return res.json(422, document);
        ...
        routes: {
            ...
            documentPost: function documentPost(req, res) {
                ...
                upsertDocumentHandler(req, function (rejectedForm, modelConfig, errors) {
                    if (rejectedForm)
                        return readyForm(rejectedForm, modelConfig, is_new, is_dialog, function (locals) {
                            res.status(422);
                            locals.errors = locals.errors || errors;
                            res.render("document.jade", locals)
                        });

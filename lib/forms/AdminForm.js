...
AdminForm.prototype.init_fields = function () {
    ...
    while (forest.length) {
        ...
        if (!children) {
            fields[vertexName] = vertexValue;
            continue;
        }
        forest = _.pairs(children)
            .map(function (p) {return [this.prefix + p[0], p[1]];}, {prefix: vertexName + '[].'}) // jshint ignore:line
            .concat(forest);
    }
...
AdminForm.prototype.render_head = function () {
    ...
    var header_lines = _(this.fields)
        ...
        .concat(this.header_lines, '<link rel="stylesheet" href="' + root + '/vendor/datetimepicker/bootstrap-datetimepicker.min.css" />')
        .concat(this.header_lines, '<link rel="stylesheet" href="' + root + '/css/forms.css" />')
        ...
        .concat(this.header_lines, '<script src="' + root + '/vendor/bootbox.min.js" ></script>')
        .concat(this.header_lines, '<script src="' + root + '/vendor/datetimepicker/bootstrap-datetimepicker.min.js" ></script>')
...
AdminForm.prototype.pre_process = function () {
    ...
    _.each(this.fields, function (field) {
        p = p.then(function () { return field.pre_process(); });
    });
    return p;
};
    ...
    return _(fields).groupBy(function (field) {
        ...
        return parts[0];
            ...
            if (group.length > 1) {
                ...
                if (renderFunc)
                    field.render_with_label = function () { renderFunc(_.omit(this, 'render_with_label'), name); };
            }
            return [name, field];
        }).object();
};

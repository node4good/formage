...
_.assign(AdminForm.prototype, {
    ...
    actual_save: function (callback) {
        ...
        self.instance.save(function (err, object) {
            ...
            var errors = err.errors || _.object([[err.path, err]]);
            self.errors = {};
    ...
    scanFields: function (form_fields) {
        ...
        Object.keys(form_fields).forEach(function (key) {
            ...
            if (value instanceof fields.RefField) {
                if ((value.options.url) && value.options.query) {
                    value.options.widget_options.url = value.options.url;
...
function getInnermostSchema(mongoose_field, tree) {
    ...
    var schema;
    if (inner_schema && (!_.isObject(inner_schema) || inner_schema.type)) {
        var single_field = _.assign({}, inner_schema, mongoose_field.options);
        single_field['type'] = inner_schema['type'] || mongoose_field.options.type[0];
        schema = new MongooseSchema({__self__: single_field});
    }
    else if (mongoose_field.options.type[0].paths && mongoose_field.options.type[0].tree) {
        schema = mongoose_field.options.type[0];
    }
    else {
        schema = new MongooseSchema(mongoose_field.options.type[0]);
    }
    return schema;
}

...
ArrayField.prototype.unbind = function () {
    var prefix = this.db_path + '_li';
    var pre_len = prefix.length;
    var grouped_data = _(this.data)
        ...
        .map(function (value, fullname) {
            ...
            var entry = { index: index, kv:[key, value] };
            return entry;
        })
        .groupBy('index')
        .reduce(function (seed, group, i) {
            seed[i] = _(group).pluck('kv').zipObject().valueOf();
            return seed;
        }, {});

    var subs = this.sub_forms,
        template = this.item_template;

    this.sub_forms = _.map(grouped_data, function (data, i) {
        // add new item

...
CompositeField.prototype.render = AdminForm.prototype.render;


    function xxx(res, prefix, value, errors) {
    var fields = this.fields;
    errors = errors || {};
    ...
    function organizeByFieldSets(fields) {
        return _.map(fields, function (field, name) {
            if (field.fields) return {title: name, fields: organizeByFieldSets(field.fields)};
            else return name;
        });
    }
    render_fieldset(organizeByFieldSets(fields), this.name);
};

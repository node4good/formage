...
var BaseField = exports.BaseField = Class.extend({
    ...
    set: function (value) {
        this.value = arguments.length ? value : this.default_value;
    },
    unbind: function () {
...
var FilepickerField = exports.FilepickerField = BaseField.extend({
    ...
    unbind: function () {
        ...
        if (this.value[this.name + '_clear']) {
            this.value = null;
        }
    }
});
...
var PictureField = exports.PictureField = BaseField.extend({
    ...
    unbind: function () {
        var data = this.data;
        if (_.isString(data)) this.value = JSON.parse(data);
        else this.value = _.omit(data, 'ws');
        if (this.value && this.value.size === 0) this.value = null;

        if (this.value && this.value.url && this.data[this.name + '_clear']) {
            this.value = null;
        }
    },
    validate: function (callback) {
        var upload_input_name = this.name + '_file';
        var thisField = this;
        if (this.data && this.data[upload_input_name] && this.data[upload_input_name].name) {
            require('cloudinary').uploader.upload(this.data[upload_input_name].path, function (result) {
                result.original_name = thisField.data[upload_input_name].name;
                result.original_size = thisField.data[upload_input_name].size;
...
var GeoPointField = exports.GeoPointField = BaseField.extend({
    ...
    unbind: function () {
        ...
        if (parts.length !== 2 || parts[0] === '' || parts[1] === '') {
            ...
            if (this.name + '_address' in this.data) {
                this.value.address = this.data[this.name + '_address'];
            }
        }
    }
});

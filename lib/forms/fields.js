...
var BaseField = exports.BaseField = Class.extend({
    ...
    unbind: function () {
        this.value = this.data[this.db_path || '__self__'];
...
var FilepickerField = exports.FilepickerField = BaseField.extend({
    ...
    unbind: function () {
        ...
        if (this.value[this.db_path + '_clear']) {
            this.value = null;
        }
    }
});
...
var PictureField = exports.PictureField = BaseField.extend({
    ...
    unbind: function () {
        var data = this.data[this.db_path] || 'null';
        this.value = JSON.parse(data);

        if (this.value && this.value.url && this.data[this.db_path + '_clear']) {
            this.value = null;
        }

        this.file = this.data[this.db_path + '_file'];
    },
    pre_process: function () {
        var self = this;
...
var GeoPointField = exports.GeoPointField = BaseField.extend({
    ...
    unbind: function () {
        ...
        if (parts.length !== 2 || parts[0] === '' || parts[1] === '') {
            ...
            if (this.db_path + '_address' in this.data) {
                this.value.address = this.data[this.db_path + '_address'];
            }
        }
    }
});

...
AdminUserForm.prototype.validate = function (callback) {
    var self = this;
    AdminForm.prototype.validate.call(this, function (err, result) {
        if (err || !result) return callback(err, result);
        var data = self.data || {current_password: '', password_again: 'x'};
        if (data.password) {
            if (this.model.compareSync(data.current_password, self.instance.passwordHash)) {
...
AdminUserForm.prototype.actual_save = function (callback) {
    if (this.data.password)
        this.instance.passwordHash = this.model.encryptSync(this.data.password);

...
exports.AviaryWidget = exports.InputWidget.extend({
    ...
    render: function (res) {
        ...
        this.value = JSON.stringify(raw_value);
        this._super(res);
        var idslug = this.name.replace('.', '_');
        res.write(util.format('<button class="btn btn-primary" id="id_%s_button">Pick File</button>', idslug));
        res.write(util.format('<a href="%s" target="_blank">%s</a>\n', raw_value.url || '#', raw_value.filename || ''));
        if (raw_value && raw_value.url) {
            res.write(util.format('<input type="checkbox" class="file-clear" name="%s_clear" value="false" /> Clear\n', idslug));
        }
        var url = raw_value.url || 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs%3D';
        res.write(util.format("<img id='id_%s_thumb' src='%s' height='110' width='150'/>\n", idslug, url));
        res.write(util.format("<script>$('#id_%s_button').click(function (e) {e.preventDefault(); trigger_filepicker('id_%s_thumb');});</script>", idslug, idslug));
    }
});

/*global escapeHTMLComponent,encodeJSONHTML */
'use strict';
...
exports.InputWidget = Widget.extend({
    ...
    render: function (res) {
        var value_str = 'value' in this ? ' value="' + escapeHTMLComponent(this.value) + '" ' : ' ';
        res.write('<input' + value_str );
...
exports.TextAreaWidget = Widget.extend({
    render: function (res) {
        ...
        this.render_attributes(res);
        res.write(' >');
        res.write(escapeHTMLComponent(value));
        res.write('</textarea>\n');
        return this;
    }
});
...
exports.FileWidget = exports.InputWidget.extend({
    ...
    render: function (res) {
        ...
        if (this.value && this.value.path) {
            res.write('<a href="' + this.value.url + '">' + this.value.path + '</a>\n<input type="checkbox" name="' + this.name + '_clear" value="Clear" /> Clear\n');
        }
    }
});
...
exports.MapWidget = exports.InputWidget.extend({
    init: function (options) {
        ...
        this.head.push('<script src="//maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&language=' + lang + '&libraries=places&key="' + GOOGLE_MAPS_API_KEY + '"></script>');
    },

...
var Widget = exports.Widget = Class.extend({
    ...
    getID: function () {return 'id_' + (this.name || '').replace('.', '_');}
});
...
exports.AviaryWidget = exports.InputWidget.extend({
    ...
    render: function (res) {
        ...
        var thumb_url = raw_value.url || 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs%3D';
        res.write(util.format(
            "<a href='%s' target='_blank' class='file-link'>\n" +
                "<img id='%s_thumb' class='thumb-picker' src='%s' height='110' width='150'/>\n" +
            "</a>\n",
        ...
        res.write(util.format(
            '<div class="btn-group">\n' +
                '<button type="button" class="btn btn-primary picker" id="%s_button1">%s</button>\n' +
                ...
                '</div>\n',
            this.getID(), pick_caption, this.getID(), show_edit, this.getID(), show_clear)
        );
        res.write(util.format("<script>" +
            "$('#%s_button1').click(trigger_filepicker);" +
            "$('#%s_button2').click(trigger_refilepicker);" +
            "$('#%s_button3').click(trigger_clearpicker);" +
            "</script>",

...
function AdminClass(options) {
    this.models = {};
    this.options = options;
};


AdminClass.prototype.registerModel = function (model, modelName, opt_options) {
    if (!model) return;
    opt_options = opt_options || {};
    if (_.all(opt_options, _.isFunction)) opt_options = {};
    var list_populate = [];
    var paths = formage.adapter.getPaths(model);
    var list = paths.map(function (options) {
        if (!options.type || !options.type.name) return null;
        if (~LIST_EXCLUDED_FIELDS.indexOf(options._path)) return null;
        if (options.type.name == 'File') return null;
        if (options.ref) {
            list_populate.push(options._path);
        }
        return options._path;
    }).compact().valueOf();
...
AdminClass.prototype.modelCounts = function (collectionName, filters, callback) {
...
AdminClass.prototype.listModelDocuments = function (modelName, start, count, filters, sort, callback) {
    ...
    return query(function (err, documents) {
        if (err) {
...
AdminClass.prototype.documentUnbinding = function (req, modelName, document, data, callback) {
...
AdminClass.prototype.createDocument = function (req, modelName, data, callback) {
...
AdminClass.prototype.updateDocument = function (req, modelName, documentId, data, callback) {
...
AdminClass.prototype.deleteDocument = function (user, modelName, documentId, callback) {
...
AdminClass.prototype.orderDocuments = function (user, modelName, data, callback) {
...
AdminClass.prototype.actionDocuments = function (user, modelName, actionId, data, callback) {
    if (!user.hasPermissions(modelName, 'update')) throw new Error('unauthorized');
...
AdminClass.prototype.userFromSessionStore = function (sessionStore) {
    return sessionStore ? formage.adapter.fromSessionStore(sessionStore) : false;
};


AdminClass.prototype.ensureUserExists = function (username, password) {
    formage.adapter.ensureExists(username, password, function (err, adminUser) {
        if (!err)
...
AdminClass.prototype.login = function (username, password, callback) {
    formage.adapter.getByUsernamePassword(username, password, callback);
};

AdminClass.prototype.registerAdminUserModel = function (name, options) {
    this.registerModel(formage.adapter.Users, name || 'Admin Users', _.extend({
    ...
    process.nextTick(function () {
        filters.forEach(function (filter) {
            ...
            model.collection.distinct(filter, function (err, results) {
                ...
                formage.dblayer.model(options.ref).find().where('_id').in(results).exec(function (err, refs) {
                    if (!refs) return;
                    output.push({
                        key: filter,
                        isString: false,
...
function mongooseSort(query, sort) {
    if (Number(formage.dblayer.version.split('.')[0]) < 3) {
        if (sort[0] === '-')

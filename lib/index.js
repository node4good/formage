...
exports.init = function (rootApp, express, models, options) {
    ...
    if (!options.no_users) {
        adapter.UsersModel = registry.registerModel(adapter.Users, 'Admin_Users', {hidden: !options.admin_users_gui});
        // Some DI magic. Sorry :(
        exports.AdminForm.DI = {getFields: adapter.getFields};
        if (!options.dont_ensure_superuser) {
            adapter.UsersModel.ensureExists(options.username, options.password, function (err, adminUser) {
                if (err) throw err;

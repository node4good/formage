...
var Mission_Progress = new Schema({
    mission_id:{type:ObjectId, ref:'Mission'},
...
var Crew_Member = new Schema({
    image: File,
    title: {type: String},
    Charachter: {charachter_id: {type: ObjectId, ref: 'Charachter'}},
...
var user = exports.user = new Schema({
    ...
    fb_access_token: String,
    current_tier_id: {type: ObjectId, ref: 'Tier'},
    ...
    fb_uniq: {type: String, index: true, dropDups: true},
    fb_profile_pic_url: String,
    last_mission_played: {mission_id: {type: ObjectId, ref: 'Mission'}},
    fanclub: {
        image: File,
        background_image: File,
        interact_background: String,
        team_id: {type: ObjectId, ref: 'Team', index: true},
        team_name: String,
    ...
    Indicators: {
        ...
        members: {type: Number, min: 0, 'default': 0, index: true},
    ...
    history: {
        events_history: [
            {
                data: Date,
                event_id: {type: ObjectId, ref: 'Fictional_Event'},
                tier_number: {type: Number, min: 1, 'default': 1},
...
user.methods.count_news = function (cb) {
    mongoose.model('User_News_Item').find({'user_fb_id': this.fb_uniq}).count(cb.bind(this));
};


user.statics.get_by_fb_id = function (fb_id, callback) { this.findOne({fb_uniq: fb_id}, callback); };
